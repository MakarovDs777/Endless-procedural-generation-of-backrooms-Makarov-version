...
function CreateRandomXYZWallsForChunk(chunkX, chunkZ, scene, chunkData) {
    ...
    for (let i = 0; i < wallCount; i++) {
        ...
        const width = 1 + Math.random() * 49; // Random width
        const height = 2; // Fixed height
        const depth = 1 + Math.random() * 49; // Random depth
        const rotX = Math.random() * Math.PI * 2;
        const rotY = Math.random() * Math.PI * 2;
        const rotZ = Math.random() * Math.PI * 2;

        const wall = BABYLON.MeshBuilder.CreateBox( // Changed to CreateBox
            `wall_${chunkX}_${chunkZ}_${i}`,
            { width: width, height: height, depth: depth }, // Provide width, height, and depth
            scene
        );
