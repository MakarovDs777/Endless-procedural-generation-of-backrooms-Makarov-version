let existingCubes = {};
const CUBE_SPACING = 43; // Изменено расстояние между кубами
...
function GenerateGroundChunks(position) {
    ...
    for (let x = chunkX - 2; x <= chunkX + 2; x++) {
        for (let z = chunkZ - 2; z <= chunkZ + 2; z++) {
            const chunkId = `${x},${z}`;
            if (!existingGroundChunks[chunkId]) {
                const groundMaterial = new BABYLON.StandardMaterial(`GroundMaterial_${chunkId}`, scene);
                const groundTexture = new BABYLON.Texture("https://i.postimg.cc/2SFksfhH/Grass.jpg", scene);
                groundTexture.uScale = 5;
                groundTexture.vScale = 5;
                groundMaterial.diffuseTexture = groundTexture;
                groundMaterial.specularColor = new BABYLON.Color3(0, 0, 0); // Убираем блики
                ...
                existingGroundChunks[chunkId] = true;
            }
        }
    }
}
...
function createRandomCube(x, z, id) {
    // Увеличиваем минимальный и максимальный размер зданий в два раза
    const minSize = 16;  //  Минимальный размер теперь 6
    const maxSizeWidthDepth = 24; // Максимальная ширина и глубина теперь 14
    const maxSizeHeight = 34;  //  Максимальная высота теперь 24

    const width = Math.random() * (maxSizeWidthDepth - minSize) + minSize;
    const depth = Math.random() * (maxSizeWidthDepth - minSize) + minSize;
    const height = Math.random() * (maxSizeHeight - minSize) + minSize;

    const cube = BABYLON.MeshBuilder.CreateBox(id, { width: width, depth: depth, height: height }, scene);
    cube.position.x = x;
    cube.position.z = z;
    cube.position.y = height / 2 - 0.5;
