...
const cameraSpeed = 10; // Скорость перемещения камеры
...
async function createScene() {
    ...
    createArrowButtons(camera); // Создаём стрелочки управления
...
// Функция для создания кнопок-стрелочек
function createArrowButtons(camera) {
    const arrows = [
        { direction: 'forward', imgSrc: 'https://i.ibb.co/XjYhjyW/Up.png', position: { left: '80%', top: '60%' } },
        { direction: 'backward', imgSrc: 'https://i.ibb.co/Db5v3Bj/Down.png', position: { left: '80%', top: '80%' } },
        { direction: 'left', imgSrc: 'https://i.ibb.co/zs7HrDF/left.png', position: { left: '75%', top: '70%' } },
        { direction: 'right', imgSrc: 'https://i.ibb.co/b6422wH/right.webp', position: { left: '85%', top: '70%' } },
    ];

    arrows.forEach(arrow => {
        const img = document.createElement('img');
        img.src = arrow.imgSrc;
        img.style.position = 'absolute';
        img.style.width = '50px'; // Размер изображения
        img.style.left = arrow.position.left;
        img.style.top = arrow.position.top;
        img.style.cursor = 'pointer';
        document.body.appendChild(img);

        img.addEventListener('click', () => {
            moveCamera(camera, arrow.direction);
        });
    });
}
